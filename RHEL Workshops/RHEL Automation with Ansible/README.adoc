...
We are going to implement one layer: the conversational domain, that is going to be a simple chatbot, and a load balancer, to implement a kind of scalable architecture..

image::apparchitecture.png[Architecture Diagram]

=== The Chatbot

This layer implements a simple logic to give a conversational platform for learning the concepts of ansible. The corpus is the Ansible glossary that can be found here: https://docs.ansible.com/ansible/latest/reference_appendices/glossary.html

The initial deployment will be with only one instance, CHB1, that conforms a conversational domain, in this case Ansible Glossary.

When more (simulation) users need to access the platform, the subsequent deployments implements other instances for coping with the demand keeping the performance.

In an elastic fashion, we can shutdown or instantiate as meany chatbot instances as needed… in an automatic way.   
The Balancer

This is a simple instance which implements a round robin scheme to access the chatbot. This layer is based on web server instance that expose an api to interact with the chatbot through port 8080.. 

The communication between this web service and the chatbot engine is through a service exposed by the chatbot instances through sockets between ports 5671 to 5675.
