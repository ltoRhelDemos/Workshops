...
Every task have some hints of what jus happened. 
...
[NOTE] 
At this stage, the filesystem is created and mounted on /home/chatbot. This directory will be used for clonning the github repository where the application is stored.

=== installing the application dependencies.

The chatbot appication is pthon 3 based, so we need to install python 3 on our remote servers. We are going to create a playbook to 
...
[NOTE]
We can execute as an ad-hoc command using ansible. For example.
...
For this, we need to clone the chatbot repository from github in the remote /home/chatbot filesystem. This directory contains all the sources and data needed to execute out service.

We also need to open ports to get the application running propoerly.
...
The first part clones the git chatbot repository to the remote directory using the module *git*. Then the ports 8080 and 9095 are opened uding the module firewalld wich needs the port, the protocol and if the action will be permanent. At the end we restart the firewall service util de module service.
