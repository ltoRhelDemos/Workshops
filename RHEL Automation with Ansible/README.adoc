...
Here we have coded a Block. A block able us to manage errors easily. We start with hosts: managedhosts as in inventory file has been set. For each IP address present in the group managedhosts ansible will execute the actions inside TASKS tag. 
...
In case one or both devices are not present, a fact is created called "proceedWithInstallation" that will be useful to execute the rest of our playbook. If this variable is set to no, further installation won't be executed. This occurs in the recue clause where we use set_fact to update the proceedWithIntsllation variable.
...
Every task have some hints of what just happened. 

* changed. Is showed when the task has been successfully executed and the action change something in the remote host. This could be that, for example, the filesystem that did not exist was created.

* ok: nothing was changed on the remote host because, by the idempotnet nature, ansible determined nothing had to be done in order to get the result expected.

* skipping: By a conditional condition, the task was skipped. In this playbook, a task that expect an error was not executed because there were no errorers in the execution.

[NOTE] 
At this stage, the filesystem was created and mounted on /home/chatbot. This directory will be used for clonning the github repository where the application is stored.

=== installing the application dependencies.

The chatbot appication is python 3 based, so we need to install python 3 on our remote servers. We are going to create a playbook to 
for installing this package, but also check and install the required libraries if needed. When we say "if needed" we refer ourselves to the fact that ansible is an *idempotent tool*. Ansible will look to get to the desired stated (installed). If the package or the  libraries are already installed any of the actions assotiated will be executed.

[NOTE] 
Also, we will need to install git in order to the remote host be able to clone the chatbot application repository.

Let's start for creating this playbook.

[NOTE] by now, we are creating independent Playbooks as big blocks as far as simplicity of explanation is concerned. At the end we will be using Ansible import_playbook directive to import every individual playbook and execute them as a whole.
...
The first two tasks installs python and git, then it continues with the actions associated with libraries installation using the *module pip*. Each one is in charge of taking the library to the state desired, by default is "installed".
...
[NOTE] 
In this stage we are sure all re pre-requisites are met for installing the appplication.
...
We also need to open ports to get the application running propoerly and create the configuration files to manage the application as a service using the systemd facility.
...
* Using the *git module* we are clonning the chatbot application repository on /home/chatbot fliesystem
* The *copy module* is used copy the two configuration files need for systemd to start, stop, enable, disable and get the status of our services, the chatbot engine and the web interface service.
* Then we use the *firewalld module* in order to open the ports 8085 and 9095 (tcp) making them permanent.
* Finnally with the *service module* we restart firewalld.
...
The playbook is executed successfully having 6 changes made, in this case the repo clonning, the systemd configuration files copyed to /etc/systemd/system directory for manipulating our programs as services, the firewalld update to open 8080 and 9095 tcp ports and the firewalld process restart.

Now our application is in the managed host ready to be executed to start the chat service. Let's first try to execute manually the engine on the managed host.
...
[NOTE]
The sintax is very simple. Use the command "chat", then the parameter "question" followed by an equal sign "=" and the question substituing the spaces by underscores "_"
...
[NOTE] 
Because this is an example service provider there is not security associated for stopping the service arbitratily.
...
Now it is time to start de application as a service and automate the application starting procedure with systemd. That is the goal of the following playbook.

=== Automating the starting of the chat service

Now that we are sure the application is installed and everything goes well its time to automate the service start at boot time.
...
Enabling the services is an easy task using *systemd module*. For both services we are going to start them with the *enable* directive in order to ansible not only to start the services but making them automatically restartable at boot time.
...
After the execution we see that both services were chaged to started.
...
This is the final step to be done in order to guaratee the service will survive to the server boot. In this case, the following playbook only execute a reboot and wait for the server to boot up.
...
The playbook uses the *reboot module* in which we are specifying to wait 30 seconds until the server is up again. Also look at the test_command parameter at the end. We are using a composed bash sheel command using systemd with is-active parameter which, is both service are up, returns 0. 
...
* *chatbotInstallSoftware.yml* for installing the appplication itself and some configuration files
...
This complete a whole installation cycle of the chatbot. But how to execute this using one playbook?.

Two ways we can use to do create a unique playbook that can be executed by ansible.
...
After this execution, the application will be up and running and the server will be accessible to interct with.

* Other way to consolidate all the playbooks created is saving all individual files in a consolidated playbook. This way only one file needs to be saved and executed. The drawbacks of this approach is the scalability. The modularity of manipulating isolated playbooks and then combined on one master using imports is much more maintainable, easy to read and scalable.

=== How to use Ansible Tower to speed up and get more control on Ansible.

From Ansible Tower Documentation we can obtain a very brief description of hat Tower is and how it can help us to be more productives.
...
The first step we need to take is installing Tower in a server. For this workshop's purpose we are going to install the Tower application in the same server as ansible was installed in the first place. For this, we propose to create a playbook for automating this installation.
...
For using Tower we need to request a Trial license in http://ansible.con/license.
...
Now it is time to create Job Template. From the Ansible Tower Documentation we have extrated:
...
Let's stablish what we need to do for creating and executing a job template. In the first place we need to create a project. From official documentation "A Project is a logical collection of Ansible playbooks, represented in Tower." So, projects are mechanisms of defining a set of playbooks to pursue a goal.

For example, in our workshop, our goal is to install the ansible chatbot appplication. So, we need to create a project where all the playbooks created previously need to be defined. For doing this, we have two mechanisms. The first one is the reccomended one which is to define a SCN repository for storing all the components of pur playbooks. 

The second method is using a local place to store the playbooks. for simplicity we are going to use this last method. 

Ansible tower maintains /var/lib/awx/projects/ in which we can crate sub-folders for our projects. We need to be sure the folders and its content are accesible by the user *awx*.
...
In precious parts of this workshop we generated a ssh key on the ansible server, then copy it to each ansible host (managedhosts) where the playbooks were going to be executed. way there is no need to authenticate againts those servers useing username and password. Now we can do the same, but instead we are going to create a credential for these servers in order to authenticate using the root account. First select Credentials at the left menu and then push the PLUS button.
...
image::tower-inventory-add.png[Push the PLUS button then Inventory in the pull down menu]

image::tower-inventory-name.png[Fill out the fields and push the save button]

image::tower-inventory-groups.png[Push the groups button and then the PLSU button]

image::tower-inventory-groups-name.png[Fill out the fields and push the save button]

image::tower-inventory-hosts.png[Now push the hosts button and the the PLUS button]

image::tower-inventory-hosts-name.png[Fill out the fields and push the save button]

image::tower-inventory-hosts-groups.png[Push the groups button and in the emergent window select managedhosts group previosly created, then press save]

image::tower-inventory-hosts-groups-assigned.png[After saving the host is assigned to the managedhosts group]
