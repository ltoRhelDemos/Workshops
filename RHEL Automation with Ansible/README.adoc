...
We are going to check the hostvars content, which is populated when the gather_facts module is automatically executed. In this case we are checking the value of the dictionary with hostvars[inventory_hostname]["ansible_facts"]["devices"]["sdb"] to determine if sdb exists.

link: https://docs.ansible.com/ansible/latest/modules/gather_facts_module.html[gather_facts module reference]
...
The trick here is use the *when* clause to chack for the existance of the device in the facts gathered. 
